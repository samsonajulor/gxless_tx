...
var init_package = __esm({
  "package.json"() {
    ...
    dependencies = {
      "@particle-network/analytics": "^0.13.5",
      "@particle-network/common": "^0.14.0",
      "@particle-network/crypto": "^0.13.5",
      draggabilly: "^3.0.0",
      qs: "^6.11.0"
    };
    devDependencies = {
...
var isBlockingThirdpartyCookiesBrowser = () => {
  return isSafariOrIOS() || isBraveBrowser() || isFireFox();
};
...
// src/utils/tron/crypto.ts
var sha256 = (msg) => import_crypto.default.createHash("sha256").update(Buffer.from(msg, "hex")).digest("hex");
...
function getWalletUrl(auth, config) {
  ...
  const ciphertext = import_crypto3.urlCrypto.encryptUrlParam(params);
  ...
  if (auth.isLogin() && auth.walletExist()) {
    if (options) {
      if (!options.walletAddress) {
        options.walletAddress = (_b = auth.wallet()) == null ? void 0 : _b.public_address;
      }
      if (!options.network) {
      ...
      options = {
        walletAddress: (_c = auth.wallet()) == null ? void 0 : _c.public_address,
...
var _WalletEntryPlugin = class {
  ...
  openWallet() {
    ...
    if (!((_b = (_a = this.auth) == null ? void 0 : _a.isLogin) == null ? void 0 : _b.call(_a))) {
      (_d = (_c = this.auth) == null ? void 0 : _c.login) == null ? void 0 : _d.call(_c);
      return;
    }
    ...
    if (!isNullish((_f = (_e = this.auth) == null ? void 0 : _e.getAuthTheme()) == null ? void 0 : _f.modalBorderRadius)) {
      iframeContent.style.borderRadius = `${(_h = (_g = this.auth) == null ? void 0 : _g.getAuthTheme()) == null ? void 0 : _h.modalBorderRadius}px`;
    }
    ...
    if ((iframe == null ? void 0 : iframe.src) && import_qs.default.parse(iframe == null ? void 0 : iframe.src.split("?").pop()).theme !== uiMode) {
      iframe.remove();
      iframe = null;
    }
    if (!iframe) {
  ...
  preload() {
    try {
      const src = `${walletUrl()}/preload/index.js?_=${+new Date()}`;
...
var Auth = class {
  ...
  login(config) {
    return __async(this, null, function* () {
      ...
      const url = yield this.buildUrl("/login", {
        ...
        login_form_mode: config == null ? void 0 : config.loginFormMode,
        ...
        this.setAuthResult({
          resolve: (value) => {
            ...
            particleActive(
              this.bi,
              this.chainId(),
              ((_a2 = this.wallet()) == null ? void 0 : _a2.public_address) || "",
              this.userInfo(),
      ...
      if (hideLoading) {
        try {
          yield silence_logout_default({
            token: ((_a = this.userInfo()) == null ? void 0 : _a.token) || "",
  ...
  accountSecurity() {
    return __async(this, null, function* () {
      ...
      const url = yield this.buildUrl("/account/security", { token: (_a = this.userInfo()) == null ? void 0 : _a.token });
      const container = yield this.openUrl(url);
      const state = new URL(url).searchParams.get("state") || "";
      return new Promise((resolve, reject) => {
        this.setAuthResult({
    ...
    return __async(this, null, function* () {
      ...
      if (this.config.chainName === "Solana") {
        ...
        url = yield this.buildUrl("/evm-chain/sign", {
          token: (_b = this.userInfo()) == null ? void 0 : _b.token,
          method,
          message
        });
      }
  ...
  switchChain(chain, hideLoading = false) {
    return __async(this, null, function* () {
      ...
      const chainInfo = import_common.chains.getChainInfo(chain);
      if (!chainInfo) {
  ...
  setChainInfo(chain, hideLoading = false) {
    return __async(this, null, function* () {
      return yield this.switchChain(chain, hideLoading);
    });
  }
  createWallet(name2, hideLoading = false) {
    return __async(this, null, function* () {
      ...
      const wallet = this.wallet(name2 === "Solana" ? "solana" : "evm_chain");
      if (wallet) {
  ...
  getEVMAddress() {
    const wallet = this.wallet("evm_chain");
    return Promise.resolve(wallet == null ? void 0 : wallet.public_address);
  }
  getSolanaAddress() {
    const wallet = this.wallet("solana");
    return Promise.resolve(wallet == null ? void 0 : wallet.public_address);
  }
  setAuthTheme(config) {
    if (config.uiMode) {
  ...
  openUrl(url, width = 475, height = 770, forcePopup = false, contentKey = "sign") {
    return __async(this, null, function* () {
      if (isBlockingThirdpartyCookiesBrowser() || forcePopup) {
  ...
  buildUrl(_0) {
    return __async(this, arguments, function* (path, extraParams = {}) {
      ...
      const state = (0, import_crypto4.uuidv4)();
      let value = import_crypto4.urlCrypto.encryptUrlParam(params);
  ...
  decrypt({ data, state }) {
    ...
    const plaintext = import_crypto4.urlCrypto.decryptData(data, secretKey, "hex");
...
var EvmService = class {
  ...
  assertEVM() {
    var _a, _b;
    if (((_b = (_a = this.auth.chain()) == null ? void 0 : _a.name) == null ? void 0 : _b.toLowerCase()) === "solana") {
  ...
  getRpcUrl() {
    ...
    return `${domain}?chainId=${this.auth.chain().id}&projectUuid=${this.auth.config.projectId}&projectKey=${this.auth.config.clientKey}`;
  }
};
...
var ParticleNetwork = class {
  constructor(config) {
    ...
    if ((!config.chainName || typeof config.chainName === "string") && (!config.chainId || typeof config.chainId === "number") && typeof config.projectId === "string" && typeof config.clientKey === "string" && typeof config.appId === "string") {
      if (config.chainName && config.chainId) {
        const chain = import_common2.chains.getChainInfo({
          id: config.chainId,
      ...
      this.config = config;
      this.bi = new import_analytics3.BI({
      ...
      if (((_b = (_a = this.config) == null ? void 0 : _a.wallet) == null ? void 0 : _b.displayWalletEntry) === void 0 || ((_d = (_c = this.config) == null ? void 0 : _c.wallet) == null ? void 0 : _d.displayWalletEntry)) {
  ...
  openActive() {
    var _a;
    if (this.auth.isLogin() && ((_a = this.auth.wallet()) == null ? void 0 : _a.public_address)) {
      particleActive(
        this.bi,
        this.auth.chainId(),
  ...
  setChainInfo(chain, hideLoading = false) {
    return this.switchChain(chain, hideLoading);
  }
  walletEntryDestroy() {
    var _a;
